...
print_help() {
    ...
    echo " -r, --repo_host	go repo host, defaults to github.com. Even if project is hosted on github, some projects want a dif host in the go dir. structure. (only for golang)"
    echo " -a, --add_alias	add the alias 'pu' to git which allows auto updating a forked repo with 'git pu'"
    echo " -u, --update		update a forked repo after cloning to match its upstream."
